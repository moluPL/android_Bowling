...
public class EditLeague extends Activity {
	...
	private Leagues_DB databaseLeague;
	...
	private Spinner spinner_league; 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.edit_league);
		init();
		disenable(false);
	}
	/**
	...
	public void init(){
		pattR_id="1";
		pattL_id="1";
		name = (EditText) findViewById(R.id.edit_league_text_name);
		home = (EditText) findViewById(R.id.edit_league_text_home);
		txt_select_patt = (TextView) findViewById(R.id.edit_league_patternselectTXT);
		btn_patterns = (Button) findViewById(R.id.edit_league_btn_select_pattern);
		txt_select_patt2 = (TextView) findViewById(R.id.edit_league_patternselectTXT2);
		btn_patterns2 = (Button) findViewById(R.id.edit_league_btn_select_pattern2);
		btn_save = (Button) findViewById(R.id.edit_league_button_save);
		btn_cancel  = (Button) findViewById(R.id.edit_league_button_cancel); 
		check = (CheckBox) findViewById(R.id.edit_league_checkBoxUnknow);
		checkSame = (CheckBox) findViewById(R.id.edit_league_checkSame);
		spinner_league = (Spinner) findViewById(R.id.edit_league_spinner_leagues);
		loadLeague();	
		...
		btn_save.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				if(getValue())
				{
					databaseLeague = new Leagues_DB(getApplicationContext());
					if(!check.isChecked()){
						if(databaseLeague.UpdateLeague(league_id,vname, vhome, pattR_id,pattL_id)){
							setResult(1);
						...
						if(databaseLeague.UpdateLeague(league_id,vname, vhome,"-1","-1")){
							setResult(1);
							Toast.makeText(getApplicationContext(), "Added to Database",Toast.LENGTH_LONG).show();
							finish();
						}
	...
	/**
	 * fill editexts to edit from DB
	 * @param league_id2
	 */
	protected void fill(int league_id2) {
		databaseLeague = new Leagues_DB(getApplicationContext());
		String[][] ligi = databaseLeague.getLeagueFromId(league_id2);
		if(ligi!=null){
			disenable(true);
			name.setText(ligi[1][1]);
			home.setText(ligi[1][2]);
			pattL_id = ligi[1][3];
			pattR_id = ligi[1][3];
			setPatterns(pattL_id, false);
			setPatterns(pattR_id, true);
	...
	/**
	 * disable/enable buttons by parameter
	 * @param b
	 */
	...
	/**
	 * coming back from selecting patterns
	 */
	...
	/**
	 * load league from DB into spinner
	 */
	public void loadLeague(){
        ...
        List<String> names = databaseLeague.getAllNames();
        if(names.size()>0)
        {
